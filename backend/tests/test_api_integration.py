...
def test_model_version_artifact_prediction_and_metrics(tmp_path: Path, monkeypatch) -> None:
    ...
    try:
        ...
        version_response = client.post(f"/api/v1/models/{model_id}/versions", json={"version": "v1", "artifact_path": "", "framework": "sklearn"})
        assert version_response.status_code == 201
        with artifact_file.open("rb") as file_handle:
            upload_response = client.post(f"/api/v1/models/{model_id}/versions/v1/artifact", files={"file": ("model.joblib", file_handle, "application/octet-stream")})
        assert upload_response.status_code == 201
        deploy_response = client.post(f"/api/v1/models/{model_id}/versions/v1/deploy")
        assert deploy_response.status_code == 200, deploy_response.text
        prediction_response = client.post(f"/api/v1/models/{model_id}/versions/v1/predict", json={"input": "I love ModelDock"})
...
def test_model_not_found_returns_404(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("MODELDOCK_API_AUTH_ENABLED", "true")
    monkeypatch.setenv("MODELDOCK_ADMIN_API_KEY", "test-admin-key")
    engine = create_engine(f"sqlite:///{tmp_path / 'not_found.db'}", connect_args={"check_same_thread": False})
    Base.metadata.create_all(engine)
    SessionTesting = sessionmaker(bind=engine, autoflush=False, autocommit=False)
...
def _create_test_model_and_version(tmp_path: Path, monkeypatch, client: TestClient, framework: str) -> tuple[int, Path]:
    engine = create_engine(f"sqlite:///{tmp_path / (framework + '.db')}", connect_args={"check_same_thread": False})
    Base.metadata.create_all(engine)
    SessionTesting = sessionmaker(bind=engine, autoflush=False, autocommit=False)
    ...
    artifact_root = tmp_path / "artifacts"
    monkeypatch.setattr(upload_artifact_store, "root", artifact_root)
    model_response = client.post("/api/v1/models", json={"name": f"validation-{framework}", "task": "test", "description": "artifact validation"})
    assert model_response.status_code == 201
    model_id = model_response.json()["id"]
    version_response = client.post(f"/api/v1/models/{model_id}/versions", json={"version": "v1", "artifact_path": "", "framework": framework})
    assert version_response.status_code == 201
    return model_id, artifact_root


def test_empty_artifact_is_rejected(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("MODELDOCK_API_AUTH_ENABLED", "true")
    monkeypatch.setenv("MODELDOCK_ADMIN_API_KEY", "test-admin-key")
    client = TestClient(app, headers={"Authorization": "Bearer test-admin-key"})
...
def test_invalid_sklearn_artifact_is_rejected(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("MODELDOCK_API_AUTH_ENABLED", "true")
    monkeypatch.setenv("MODELDOCK_ADMIN_API_KEY", "test-admin-key")
    client = TestClient(app, headers={"Authorization": "Bearer test-admin-key"})
    model_id, _ = _create_test_model_and_version(tmp_path, monkeypatch, client, "sklearn")
    response = client.post(f"/api/v1/models/{model_id}/versions/v1/artifact", files={"file": ("invalid.joblib", b"this is not a joblib model", "application/octet-stream")})
    assert response.status_code == 422
    app.dependency_overrides.clear()


def test_unsupported_framework_is_rejected(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("MODELDOCK_API_AUTH_ENABLED", "true")
    monkeypatch.setenv("MODELDOCK_ADMIN_API_KEY", "test-admin-key")
    client = TestClient(app, headers={"Authorization": "Bearer test-admin-key"})
...
def test_replacing_artifact_invalidates_runtime_cache(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("MODELDOCK_API_AUTH_ENABLED", "true")
    monkeypatch.setenv("MODELDOCK_ADMIN_API_KEY", "test-admin-key")
    engine = create_engine(f"sqlite:///{tmp_path / 'replacement.db'}", connect_args={"check_same_thread": False})
    Base.metadata.create_all(engine)
    SessionTesting = sessionmaker(bind=engine, autoflush=False, autocommit=False)
    ...
    try:
        client = TestClient(app, headers={"Authorization": "Bearer test-admin-key"})
        model_response = client.post("/api/v1/models", json={"name": "replacement-test", "task": "test", "description": "artifact replacement"})
        assert model_response.status_code == 201
        model_id = model_response.json()["id"]
        ...
        first_upload = client.post(f"/api/v1/models/{model_id}/versions/v1/artifact", files={"file": ("model_a.py", artifact_a, "text/plain")})
        assert first_upload.status_code == 201
        first_path = first_upload.json()["artifact_path"]
        deploy_response = client.post(f"/api/v1/models/{model_id}/versions/v1/deploy")
        assert deploy_response.status_code == 200, deploy_response.text
        first_prediction = client.post(f"/api/v1/models/{model_id}/versions/v1/predict", json={"input": "hello"})
        ...
        undeploy_response = client.post(f"/api/v1/models/{model_id}/versions/v1/undeploy")
        assert undeploy_response.status_code == 200, undeploy_response.text
        assert resolved_first_path not in runtime._cache
        second_upload = client.post(f"/api/v1/models/{model_id}/versions/v1/artifact", files={"file": ("model_b.py", artifact_b, "text/plain")})
        assert second_upload.status_code == 201
        second_path = second_upload.json()["artifact_path"]
        assert second_path != first_path
        assert not Path(first_path).exists()
        redeploy_response = client.post(f"/api/v1/models/{model_id}/versions/v1/deploy")
        assert redeploy_response.status_code == 200, redeploy_response.text
        second_prediction = client.post(f"/api/v1/models/{model_id}/versions/v1/predict", json={"input": "hello"})
...
def test_failed_inference_records_error(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("MODELDOCK_API_AUTH_ENABLED", "true")
    monkeypatch.setenv("MODELDOCK_ADMIN_API_KEY", "test-admin-key")
    engine = create_engine(f"sqlite:///{tmp_path / 'failure_metrics.db'}", connect_args={"check_same_thread": False})
    Base.metadata.create_all(engine)
    SessionTesting = sessionmaker(bind=engine, autoflush=False, autocommit=False)
    ...
    try:
        client = TestClient(app, headers={"Authorization": "Bearer test-admin-key"})
        model_response = client.post("/api/v1/models", json={"name": "failure-metrics-test", "task": "text-classification", "description": "failure metrics"})
        assert model_response.status_code == 201
        model_id = model_response.json()["id"]
        version_response = client.post(f"/api/v1/models/{model_id}/versions", json={"version": "v1", "artifact_path": "", "framework": "json"})
        assert version_response.status_code == 201
        upload_response = client.post(f"/api/v1/models/{model_id}/versions/v1/artifact", files={"file": ("model.json", b'{"predictions": {"hello": "positive"}}', "application/json")})
        assert upload_response.status_code == 201
        deploy_response = client.post(f"/api/v1/models/{model_id}/versions/v1/deploy")
        assert deploy_response.status_code == 200, deploy_response.text
        prediction_response = client.post(f"/api/v1/models/{model_id}/versions/v1/predict", json={"input": "unknown-input"})
        assert prediction_response.status_code == 422
        history_response = client.get(f"/api/v1/metrics/{model_id}/v1/history")
...
def test_deployed_version_cannot_replace_artifact(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("MODELDOCK_API_AUTH_ENABLED", "true")
    monkeypatch.setenv("MODELDOCK_ADMIN_API_KEY", "test-admin-key")
    engine = create_engine(f"sqlite:///{tmp_path / 'deployed_replacement.db'}", connect_args={"check_same_thread": False})
    Base.metadata.create_all(engine)
    SessionTesting = sessionmaker(bind=engine, autoflush=False, autocommit=False)
    ...
    try:
        client = TestClient(app, headers={"Authorization": "Bearer test-admin-key"})
        model_response = client.post("/api/v1/models", json={"name": "immutable-deployed-version", "task": "test", "description": "deployed version replacement guard"})
        assert model_response.status_code == 201
        model_id = model_response.json()["id"]
        version_response = client.post(f"/api/v1/models/{model_id}/versions", json={"version": "v1", "artifact_path": "", "framework": "python"})
        assert version_response.status_code == 201
        first_upload = client.post(f"/api/v1/models/{model_id}/versions/v1/artifact", files={"file": ("model_a.py", b'def model(value):\n    return "model-a"\n', "text/plain")})
        assert first_upload.status_code == 201
        deploy_response = client.post(f"/api/v1/models/{model_id}/versions/v1/deploy")
        assert deploy_response.status_code == 200
        replacement = client.post(f"/api/v1/models/{model_id}/versions/v1/artifact", files={"file": ("model_b.py", b'def model(value):\n    return "model-b"\n', "text/plain")})
...
def test_prediction_after_undeploy_is_rejected(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("MODELDOCK_API_AUTH_ENABLED", "false")
    engine = create_engine(f"sqlite:///{tmp_path / 'undeploy_prediction.db'}", connect_args={"check_same_thread": False})
    Base.metadata.create_all(engine)
    SessionTesting = sessionmaker(bind=engine, autoflush=False, autocommit=False)
    ...
    try:
        client = TestClient(app)
        model_response = client.post("/api/v1/models", json={"name": "undeploy-test", "task": "test", "description": "undeploy prediction guard"})
        assert model_response.status_code == 201
        model_id = model_response.json()["id"]
        version_response = client.post(f"/api/v1/models/{model_id}/versions", json={"version": "v1", "artifact_path": "", "framework": "python"})
        assert version_response.status_code == 201
        upload_response = client.post(f"/api/v1/models/{model_id}/versions/v1/artifact", files={"file": ("model.py", b"def model(value):\n    return value\n", "text/plain")})
        assert upload_response.status_code == 201
        deploy_response = client.post(f"/api/v1/models/{model_id}/versions/v1/deploy")
        assert deploy_response.status_code == 200
        undeploy_response = client.post(f"/api/v1/models/{model_id}/versions/v1/undeploy")
