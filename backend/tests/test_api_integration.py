...
def test_deployed_version_cannot_replace_artifact(
    tmp_path: Path,
    monkeypatch,
) -> None:
    monkeypatch.setenv("MODELDOCK_API_AUTH_ENABLED", "true")
    ...
    database_url = f"sqlite:///{tmp_path / 'deployed_replacement.db'}"
    engine = create_engine(
        database_url,
        connect_args={"check_same_thread": False},
    )
    ...
    try:
        ...
        model_response = client.post(
            "/api/v1/models",
            json={
                "name": "deployed-replacement-test",
                "task": "test",
                "description": "deployed replacement guard",
            },
        )
        assert model_response.status_code == 201
        model_id = model_response.json()["id"]
        ...
        assert "deployed" in replacement_response.json()["detail"].lower()
    finally:
        app.dependency_overrides.clear()


def test_prediction_after_undeploy_is_rejected(
    tmp_path: Path,
    monkeypatch,
) -> None:
    monkeypatch.setenv("MODELDOCK_API_AUTH_ENABLED", "false")

    database_url = f"sqlite:///{tmp_path / 'undeploy_prediction.db'}"
    engine = create_engine(
        database_url,
        connect_args={"check_same_thread": False},
    )
    ...
    try:
        ...
        model_response = client.post(
            "/api/v1/models",
            json={
                "name": "undeploy-prediction-test",
                "task": "test",
                "description": "undeploy prediction guard",
            },
        )
        assert model_response.status_code == 201
        model_id = model_response.json()["id"]
