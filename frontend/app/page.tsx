...
export default function Home() {
  ...
  >({});
  const [predictionLatency, setPredictionLatency] = useState<
    ...
    const entries = await Promise.all(
      modelVersions.map(async (version) => {
        const response = await apiFetch(
          `/api/v1/metrics/${modelId}/${version.version}`,
        );

        return response.ok
          ? ((await response.json()) as Metrics)
          : null;
      }),
    );
  ...
  const loadHistory = async (
    ...
    const entries = await Promise.all(
      modelVersions.map(async (version) => {
        const response = await apiFetch(
          `/api/v1/metrics/${modelId}/${version.version}/history?limit=50`,
        );

        return response.ok
          ? ((await response.json()) as InferenceHistory[])
          : [];
      }),
    );
    ...
    modelVersions.forEach((version, index) => {
      map[version.version] = entries[index];
    });
  ...
  const loadTimeseries = async (
    ...
    const entries = await Promise.all(
      modelVersions.map(async (version) => {
        const response = await apiFetch(
          `/api/v1/metrics/${modelId}/${version.version}/timeseries?hours=24`,
        );

        return response.ok
          ? ((await response.json()) as TimeseriesPoint[])
          : [];
      }),
    );
  ...
  const loadModelData = async (
    ...
    await Promise.all([
      loadMetrics(modelId, modelVersions),
      loadHistory(modelId, modelVersions),
      loadTimeseries(modelId, modelVersions),
    ]);
  };
  ...
  const viewModel = async (model: Model) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("Failed to load model versions");
      }

      const modelVersions = (await response.json()) as ModelVersion[];

      setVersions(modelVersions);

      await loadModelData(model.id, modelVersions);
    } catch (err) {
      setError(
  ...
  const createVersion = async (event: FormEvent) => {
    ...
    try {
      const response = await apiFetch(
        `/api/v1/models/${selectedModel.id}/versions`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (!response.ok) {
        throw new Error(body.detail || "Failed to create version");
      }
  ...
  const deleteModel = async () => {
    ...
    const confirmed = window.confirm(
      `Delete model "${selectedModel.name}" and all of its versions, artifacts, and inference history? This cannot be undone.`,
    );
    ...
    try {
      const response = await apiFetch(
        `/api/v1/models/${selectedModel.id}`,
        {
          method: "DELETE",
        },
      );
      ...
      if (!response.ok) {
        ...
        throw new Error(
          body.detail || "Failed to delete model",
        );
      }
  ...
  const deleteVersion = async (version: ModelVersion) => {
    ...
    const confirmed = window.confirm(
      `Delete version "${version.version}" and its artifact, metrics, and inference history? This cannot be undone.`,
    );
    ...
    try {
      const response = await apiFetch(
        `/api/v1/models/${selectedModel.id}/versions/${version.version}`,
        {
          method: "DELETE",
        },
      );
      ...
      if (!response.ok) {
        const body = await response.json().catch(() => ({}));
        throw new Error(
          body.detail || "Failed to delete version",
        );
      }
  ...
  const uploadArtifact = async (
    ...
    try {
      ...
      const response = await apiFetch(
        `/api/v1/models/${selectedModel.id}/versions/${version.version}/artifact`,
        {
          method: "POST",
          body: formData,
        },
      ...
      if (!response.ok) {
        throw new Error(
          body.detail || "Failed to upload artifact",
        );
      }
  ...
  const runPrediction = async (version: ModelVersion) => {
    ...
    if (!input) {
      setPredictionErrors((current) => ({
        ...current,
        [version.version]:
          "Enter an input before running prediction.",
      }));
    ...
    try {
      const response = await apiFetch(
        `/api/v1/models/${selectedModel.id}/versions/${version.version}/predict`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  if (selectedModel) {
    return (
      <main className="dashboard">
        ...
        <section className="section-header action-header">
          <div>
            <h2>Versions</h2>
            <p>
              {versions.length} registered version
              {versions.length === 1 ? "" : "s"}
            </p>
          </div>
        ...
        <section className="grid">
          {versions.map((version) => {
            const m = metrics[version.version];
            const prediction =
              predictions[version.version];
            const predictionError =
              predictionErrors[version.version];
            const isPredicting =
              predicting[version.version];
              ...
              >
                ...
                <div className="metrics">
                  ...
                  <div>
                    ...
                    <strong>
                      {m?.average_latency_ms ?? 0} ms
                    </strong>
                  </div>
                </div>

                <MetricsCharts
                  data={timeseries[version.version] ?? []}
                />
                ...
                <button
                  type="button"
                  className="danger-button version-delete-button"
                  onClick={() =>
    ...
    <main className="dashboard">
      <header className="header">
        <div>
          ...
          <p className="subtitle">
            Manage and serve your machine learning
            models.
          </p>
        </div>
      ...
      <section className="section-header action-header">
        <div>
          ...
          <p>
            {models.length} registered model
            {models.length === 1 ? "" : "s"}
          </p>
        </div>
      ...
      {showCreateModel && (
        <form
          className="management-form model-form"
        ...
        >
          ...
          <input
            required
            placeholder="Task (e.g. text-classification)"
            value={modelForm.task}
            onChange={(event) =>
              setModelForm({
                ...modelForm,
